...
export async function getSunioTrxQuotes({
  amountIn,
  fromTokenAddress,
  tokenAddresses = {},
  inputDecimals = 6,
  ...
  if (!isUsableAddress(fromTokenAddress)) {
    throw new Error('SUN.io TRX quotes: fromTokenAddress is invalid');
  }

  const toTokenParam = tokenAddresses.TRX || SUNIO_TOKEN_ADDRESSES.TRX;

  if (!isUsableAddress(toTokenParam)) {
    throw new Error('SUN.io TRX quotes: target token address is invalid');
  }

  const amountInRaw = decimalToRaw(amountIn, inputDecimals).toString();
  const resolvedOutputDecimals = getOutputDecimalsByTarget('TRX', outputDecimals);
  ...
  if (!response.ok) {
    throw new Error(`SUN.io TRX quotes failed with status ${response.status}`);
  }

  const payload = await response.json();

  if (!payload || Number(payload.code) !== 0 || !Array.isArray(payload.data)) {
    throw new Error(payload?.message || 'SUN.io TRX quotes returned invalid payload');
  }
